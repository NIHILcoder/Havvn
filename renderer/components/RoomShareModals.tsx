...
export const TransferPickerModal: React.FC<TransferPickerModalProps> = ({ roomId, onClose, onShared }) => {
  ...
  const share = async (download: Download, selectedPaths?: string[]) => {
    ...
    try {
      // A multi-file share auto-folders under the download's name; the engine
      // creates/reuses the folder and assigns exactly the files it adds (no racy
      // renderer diff, and re-sharing reuses the same-named folder).
      const folderName = (selectedPaths?.length ?? 0) > 1 ? download.name : undefined;
      const state = await window.api.rooms.shareDownload(roomId, download.id, selectedPaths, folderName);
